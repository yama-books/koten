...
test('restore: 誘いに範囲全体の残りと何回目かを出す', async () => { const view = await mount(session({ to: 100 }), Array.from({ length: 79 }, (_, index) => event(index + 1))); expect(view.textContent).toContain('あと21首'); expect(view.textContent).toContain('5回目 / 全5回'); expect(view.textContent).not.toContain('まとまり'); });

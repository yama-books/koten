...
test('result: まちがえた歌だけをもう一度に対象の首を渡す', async () => { let received: readonly number[] = []; const view = mount(base, { onRetryWeak: (cards) => { received = cards; }, onRetrySame: () => {}, onHome: () => {} }); await act(() => { Array.from(view.querySelectorAll('button')).find((button) => button.textContent === 'まちがえた歌だけをもう一度')!.dispatchEvent(new MouseEvent('click', { bubbles: true })); }); expect(received).toEqual([3, 5]); });
test('result: 要確認がないとき再確認ボタンを出さない', () => { const view = mount({ ...base, retryCardNumbers: [] }); expect(view.textContent).not.toContain('まちがえた歌だけをもう一度'); });
...
test('result: 再確認は渡された飛び飛びの首を保つ', async () => { let received: readonly number[] = []; const view = mount({ ...base, retryCardNumbers: [3, 5] }, { onRetryWeak: (cards) => { received = cards; }, onRetrySame: () => {}, onHome: () => {} }); await act(() => { Array.from(view.querySelectorAll('button')).find((button) => button.textContent === 'まちがえた歌だけをもう一度')!.click(); }); expect(received).toEqual([3, 5]); });

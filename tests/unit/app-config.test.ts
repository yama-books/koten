...
test('P1: only QR sync is enabled', () => {
  assert.equal(appConfig.features.sync, true);
  assert.ok(Object.entries(appConfig.features).every(([name, flag]) => name === 'sync' || flag === false));
});

...
export function History({ summary, onHome, port, onChanged }: Props) {
  ...
  return <main class="history-screen"><header class="nav-edge"><span class="wordmark">これまでの記録</span><button type="button" onClick={onHome}>ホームへ戻る</button></header><p>着手した歌: {summary.touchedCount}首</p>{port && <RecordTransfer port={port} onChanged={onChanged} />}<section aria-labelledby="review-heading"><h1 id="review-heading">要確認の歌</h1><p>最後に解いたとき、まちがえたか「わからない」を選んだ歌です。</p>{summary.needsReview.length === 0 ? <p>要確認の歌はありません</p> : <ul class="history-list">{summary.needsReview.map((entry) => <Entry key={entry.poemId} entry={entry} />)}</ul>}</section><section aria-labelledby="all-heading"><h2 id="all-heading">全100首</h2><ul class="history-list">{summary.entries.map((entry) => <Entry key={entry.poemId} entry={entry} />)}</ul></section></main>;
}

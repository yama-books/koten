...
const labels: Record<EntryId, string> = { quick: 'すぐに始める', view: '歌を確認する', learn: '読みを確認しながら穴埋め', review: 'もう一度確認する', exam: '読みを隠して穴埋め' };
...
export function RangePicker({ entry, range, order, onStart, onBack }: Props) {
  ...
  return <main class="range-picker"><header class="nav-edge"><span class="wordmark">開始前の確認</span><button type="button" onClick={onBack}>戻る</button></header>
    ...
    {<section class="answer-mode" aria-labelledby="answer-mode-heading"><h2 id="answer-mode-heading">答え方</h2><div><button type="button" aria-pressed={answerMode === 'screen'} onClick={() => setAnswerMode('screen')}>画面で答える</button><button type="button" aria-pressed={answerMode === 'paper'} onClick={() => setAnswerMode('paper')}>紙に書く</button></div><p>{answerMode === 'paper' ? '1問ずつ答えを開き、自分で採点します。' : '入力した答えを1問ずつ自動で採点します。'}</p></section>}

...
type Props = { questions: readonly PublishedQuestion[]; sessionId: string; port: ApplicationPort; settings: UserSettings; onSettings: (settings: UserSettings) => void; onComplete: () => void };
...
export function Session({ questions, sessionId, port, settings, onSettings, onComplete }: Props) {
  ...
  if (!question || flow.phase === 'complete') return <main class="session"><h1>今回の範囲を確認しました</h1><button type="button" onClick={onComplete}>ホームへ戻る</button></main>;

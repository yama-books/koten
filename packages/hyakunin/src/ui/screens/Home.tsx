...
type Props = {
  ...
  onQuickStart?: (range: { from: number; to: number }, questions: PublishedQuestion[], poems: Poem[]) => void;
...
export function Home({ port, onPickEntry, onQuickStart, onResume, onOpenHistory, poems: suppliedPoems, questions: suppliedQuestions }: Props) {
  ...
  function startView(authors = false) {
    setAuthorPractice(authors); setAuthorOpen(false);
  ...
  if (viewing && poem) {
    ...
    return <main class="viewer"><header class="nav-edge"><span class="wordmark">{authorPractice ? '作者名を確認する' : '歌を確認する'}</span><span class="progress" aria-live="polite"><span class="poem-number">{poem.cardNo}</span> · {index + 1}/{selected.length}歌</span><button type="button" onClick={returnToRangeSelection}>範囲を選び直す</button></header><section class="reading-controls"><ReadingToggle value={settings.reading} onChange={(reading) => persist({ ...settings, reading })} /><WritingModeToggle value={settings.writing} onChange={(writing) => persist({ ...settings, writing })} /></section><article class={`poem-sheet poem-sheet--${settings.writing}`} aria-labelledby="poem-title"><h1 id="poem-title" class="sr-only">{poem.cardNo}{!authorPractice && ` ${poem.author.canonical}`}</h1><div class="poem" lang="ja"><div class="poem__half" aria-label={`上の句 ${ku.slice(0, 3).join(' ')}`}>{ku.slice(0, 3).map((line) => <span key={line}>{line}</span>)}</div><div class="poem__half" aria-label={`下の句 ${ku.slice(3).join(' ')}`}>{ku.slice(3).map((line) => <span key={line}>{line}</span>)}</div></div><div class="author">{!authorPractice || authorOpen ? <><strong>{poem.author.canonical}</strong>{authorReading && <span>{authorReading}</span>}</> : <button type="button" onClick={() => setAuthorOpen(true)}>作者名を見る</button>}</div></article><nav class="pager" aria-label="歌を移動"><button type="button" disabled={index === 0} onClick={() => { setCurrent(selected[index - 1].cardNo); setAuthorOpen(false); }}>前の歌</button><button type="button" disabled={index === selected.length - 1} onClick={() => { setCurrent(selected[index + 1].cardNo); setAuthorOpen(false); }}>次の歌</button></nav></main>;
  }
  return <main class="home">{settingsLoaded && !settings.noticeConfirmed && <div class="onboarding" role="region" aria-label="初回設定"><div class="onboarding__panel"><StatsNotice onConfirm={confirmNotice} /><GradePicker value={pendingGrade} onChange={setPendingGrade} /></div></div>}<header class="nav-edge"><span class="wordmark">{appConfig.products.hyakunin.displayName}</span></header>{initialRange.hadInvalidQuery && <p class="review-note" role="status">範囲を読み込めなかったため、全範囲を表示しています。</p>}{shouldOfferRestore && <section class="review-note restore-offer"><p>前回の学習を復元しますか。</p><p>あと{restorable.plan.remainingInRange}首 · {restorable.plan.chunkIndex + 1}回目 / 全{restorable.plan.chunkCount}回</p><button type="button" onClick={() => { onResume?.(restorable.session, restorable.plan.cardNumbers, questions, poems); setRestoring(false); }}>復元する</button><button type="button" onClick={() => setRestoring(false)}>復元しない</button></section>}<section class="range-panel"><h2>範囲</h2><div class="range-fields"><label><input aria-label="最初の番" type="number" min="1" max="100" value={from} onInput={(event) => setFrom(Number(event.currentTarget.value))} /><span aria-hidden="true">番</span></label><span aria-hidden="true">〜</span><label><input aria-label="最後の番" type="number" min="1" max="100" value={to} onInput={(event) => setTo(Number(event.currentTarget.value))} /><span aria-hidden="true">番</span></label></div><div class="entry-actions"><button class="primary" type="button" disabled={!isEntryAvailable('learn', questions.length)} aria-disabled={!isEntryAvailable('learn', questions.length)} onClick={() => onQuickStart?.(normalizeRange(from, to), questions, poems)}>とりあえず始める</button><button type="button" onClick={() => choose('view')}>歌を確認する</button></div><p class="entry-help">穴埋め問題から始めます。</p><button type="button" aria-expanded={practiceOpen} onClick={() => setPracticeOpen((open) => !open)}>学習方法を選ぶ</button>{practiceOpen && <div class="practice-choices" aria-label="穴埋めの方法"><div class="practice-choice"><button type="button" onClick={() => choose('learn')}>読みを確認しながら穴埋め</button><p>読みを切り替えながら覚えます。</p></div><div class="practice-choice"><button type="button" onClick={() => choose('exam')}>読みを隠して穴埋め</button><p>歌番号と読みを隠して力試し。</p></div><div class="practice-choice"><button type="button" onClick={() => startView(true)}>作者名を確認する</button><p>歌から作者を思い出し、名前を開いて確認します。</p></div></div>}{questions.length === 0 && <p role="status">問題はまだ準備中です。いまは「歌を確認する」を使えます。</p>}</section><button type="button" onClick={onOpenHistory}>これまでの記録</button><footer class="foot-line"><p class="release-stage">{releaseStageLabel}</p><details class="known-limits"><summary>この版でまだできないこと</summary><ul>{KNOWN_LIMITATIONS.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul></details><p>{appConfig.publisher} · 100首収録</p></footer></main>;
}

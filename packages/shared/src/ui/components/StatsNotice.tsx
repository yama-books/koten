...
export function StatsNotice({ onConfirm, disabled = false }: Props): JSX.Element {
  return <section class="stats-notice" aria-labelledby="stats-notice-title"><h2 id="stats-notice-title">はじめに</h2><p>{STATS_NOTICE_TEXT}</p><p id="stats-notice-grade-help" class="stats-notice__help">学年を選ぶと確認できます。</p><button class="stats-notice__confirm" type="button" disabled={disabled} aria-describedby={disabled ? 'stats-notice-grade-help' : undefined} onClick={onConfirm}>確認する</button></section>;
}

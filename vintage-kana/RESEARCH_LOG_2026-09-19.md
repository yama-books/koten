...
## 18. Phase 2C-2 context-checked 10件へ向けた witness 多様化

一次公開を見据え、次の節目を `context-checked=10` とし、同一 witness・同一音価への偏りを避ける。

### 伊勢物語嵯峨本第二種の扱い

`issg001` は国語研公開原画像と字形DBの個別位置までは取得できるが、
同一 witness の翻刻本文が独立TXTではなく viewer 側に埋め込まれており、
現行のGitHub/Web-only取得経路では文字列を安全に抽出できない。

このため、別伝本の本文で代用せず、推測による exact-text 化も行わない。

### 比翼連理花廼志満台 初編上へ切替

同一底本の国語研公式翻刻TXTが存在する `hnsd001` を次の Stage B 候補とする。

候補字体:
- ね / 字母「年」 / U+1B094 𛂔

初編上で確認した個別出現候補:
- hnsd001-014 / ID0052 / X1142 / Y0718
- hnsd001-015 / ID0119 / X1611 / Y1645
- hnsd001-026 / ID0111 / X0522 / Y1719
- hnsd001-036 / ID0103 / X1181 / Y1131
- hnsd001-039 / ID0054 / X0627 / Y2254

国語研高解像度JPEGへの直接URLも確認済み:
`https://dglb01.ninjal.ac.jp/ninjaldl/hananosimadai/001/jpg/hnsd001-<page>.jpg`

Google Slidesを取得ブリッジにすると、上記5ページの原画像実体を取得できることを確認した。
Googleは取得補助であり、証拠資料は国語研原画像と公式翻刻である。

### 未解決点

公式翻刻 `hnsd-001.txt` は公開されているが、現環境では長大TXT本文の抽出が安定しない。
Google Sheets `IMPORTDATA` も外部URLアクセス許可エラーで失敗した。

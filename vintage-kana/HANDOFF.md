...
- 「ん」の補足分布を収録するかの扱い確定
- 実文脈採取
- 「ん」の補遺調査は完了（15資料・15非ゼロレコード）。0件補完・比率は本体と同じく未処理
...
1. 47音価について、欠落 witness/glyph 組を0件として明示的に補完する
2. 資料内比率の分母を定義し、`kana_total` / `ratio` を計算する
3. `sources.json` の年代・ジャンル・所蔵・利用条件を確定する
4. 「ん」を中核47音価とは分けた補足データとして扱うか決定する
5. 高頻度字体・特徴的字体から実文脈を採取する
6. `attested-examples.json` に保存する
7. 字母・字体クイズ候補を生成する

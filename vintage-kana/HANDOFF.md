...
Phase 1 の字体分布収集は「あ・い・う・え・お・か・き・く・け・こ・さ・む・わ」の13仮名まで完了し、`data/glyph-distribution.json` に492件の非ゼロ分布レコードを保存している。
...
1. 残り34仮名の資料別字体分布を `glyph-distribution.json` に追加
2. 47仮名完了後、0件を明示的に補完
3. 分母を定義して資料内比率を計算

...
## 56. 2026-09-21 Web-only再開・配色G確定と監査残件反映

ユーザー指示により、この工程は **ローカルを一切使用せず**、GitHub `vintage-kana-main` 上だけで実施した。
配色は監査案 **G「空と桜」** を正式採用した。F/E/Hは不採用とし、以後Gを現行仕様とする。
...
- 選択中の文字・枠: 青 `#356b9e`
- 主ボタン: 桜 `#c2506e` + 白文字
- 誤答系は主ボタンの桜と意味が混ざらないよう、橙寄りへ分離
  - danger: `#a85732`
  - wrong background: `#fff0e8`
  - wrong text: `#6f3a26`
- 正答色と習熟度リングの5色境界は変更しない。
- 選択中UIは「濃色ベタ＋白」ではなく「淡い空色面＋濃い青文字」とする。
- 主ボタンだけ桜ベタ＋白文字とする。
- 背景にはごく薄い空・桜のradial gradientを加え、紙色 `#f2efe8` 自体は維持する。
- Canvas保存画像の署名色も青 `#356b9e` へ同期した。

コントラスト確認:
- `#356b9e` on `#e0ecf8`: 約4.68:1
- 白 on `#c2506e`: 約4.50:1

実装:
- `8a1756f237316ef4021eb12fde364f4964cb7b4c` — G配色本体
- `aaded8f2970199d3a07589f1c1885f7959b3302e` — G配色回帰テスト
- 両コミットのCIは success。

### ローカル側だけにあった監査済み字形修正の復元

引継ぎで完了済みとされていた `91ae986` / `17312e1` はGitHub側には存在せず、
`vintage-kana-main` の `ui-glyph-master.json` と `FALLBACK_GLYPHS` にも反映されていなかった。
ローカルへ触れず、引継ぎに明記された確定差分だけをGitHub側へ再適用した。

- U+1B0D8: `も / 毛`
- U+1B0E6: 字母 `遊`
- U+1B10C: `inOfficialCatalog:false`
- JSON本体と埋め込みfallbackを同値にした。
- unit testで3件を固定した。

### glyphInfoDialog の字形注記

`inOfficialCatalog:false` の字形を開いた場合だけ、次の注記を表示する。

> ※この字形は、国立国語研究所「学術情報交換用変体仮名」の公式収録一覧には含まれないため、補足字形として表示しています。

「公式収録一覧にない」と「アプリで利用できない」を混同しない表現とした。
通常の公式収録字形では注記を非表示にする。

### 読みラベルと習熟度円の接触をフォント非依存化

従来の `check:vintage-kana` はGoogle Fontsを意図的にabortし、fallback fontで測る。
そのため「左右または上下のどこかで非接触」という検査は、fallback glyph幅の違いで結果が変わり得た。

今回:
- カード上部に明示的な予約領域を設けた。
- 中央コアを下方向へ逃がした。
- QAを文字幅依存の「非接触」から、**読みラベル下端 → 円上端の縦ギャップ4px以上**へ変更した。
- 初回実装では390pxで0.92pxしかなくQAが赤になったため、CSSを再調整した。
- `07955be69827fafad03d97c89aa46412db780cb7` で `check:vintage-kana` が success。
- これによりLinux/Windows等のfallback文字幅差で偶然PASSする設計をやめた。

関連:
- `5fe997abd6a47bb3487040fc23fb28b3e9f44d67` — 3字形修正、字形注記、縦ギャップQA導入
- `07955be69827fafad03d97c89aa46412db780cb7` — 4pxクリアランス確保

### 導線バグ1・2・4について

前回引継ぎには「導線バグ1・2・4」とだけ記載されているが、
現在のGitHub文書・コードからは各番号の具体的な症状を一意に復元できなかった。
推測で別箇所を変更すると回帰を作るため、この3件だけは **未定義のまま変更していない**。
具体定義を確認できる資料が見つかった時点で番号と症状を対応させて再開する。

### 公開状態

- 今回の作業は `vintage-kana-main` のみ。
- `main` へ直接pushしていない。
- force-pushしていない。
- ローカルは参照・変更とも行っていない。
...
## 59. 2026-09-21 ローカル実装・4回公開・セッション区切り

§58 の続き。**§58 に書いた「push権限がなく公開できない」状態は解消済み**で、
この節の内容はすべて GitHub Pages へ公開済み。§58 の「公開が止まっている理由」は
もう現状ではないので、そちらを先に読んで混乱しないこと。

### push が通るようになった経緯

§58 時点では HTTPS remote の資格情報に書き込み権限がなく、`git push` が 403 だった。
ユーザーが **SSH鍵を発行**し、remote が `git@github.com:yama-books/koten.git` へ切り替わった。
以後 push は通る。`gh` CLI はこの環境に入っていないため、**PR作成だけは引き続き手作業**。

PRを出すときは push 後に出る `https://github.com/yama-books/koten/pull/new/<branch>` を使う。

### ブランチの状態（重要）

- **開発正本: `vintage-kana-main`（＝ローカルの作業ブランチ `vintage-kana-resume-20260921`）**
- ローカルの `vintage-kana-main` ref は **`91ae986` のまま取り残してある**（§58参照）。
  こちらは使わない。正本は `origin/vintage-kana-main`。
- `backup-local-ninjal-merge-20260921`（`91ae986`）に、GitHub側へ未統合の
  NINJAL公式カタログ293件・ツール・テストが入っている。`vintage-kana-ninjal-audit` にも同じものがある。

作業ディレクトリ:
```
D:\dev\koten-vintage-kana   ← 開発（npm ci 済み）
D:\dev\koten-publish-tmp    ← 公開ブランチ組み立て用
```

### この節で実装したこと

#### 1. 小書き仮名のチェックを全小文字へ（PR #17）

従来は拗音「ゃゅょ」3字だけが対象で、「ぁぃぅぇぉっゎ」はチェックに関係なく常に小書きだった。
`SMALL_BASE` の全10字を対象にし、**チェックなし＝すべて大文字／チェックあり＝すべて小書き**へ統一。
`YOON_SMALL`→`SMALL_TOGGLE_KANA`、`yoonSmallEnabled`→`smallKanaEnabled` に改名。
文言は「ぁ・ぃ・ゅ などを小さく表示」。

#### 2. 習熟度カードの % が切れる（PR #17）

`.glyphMasteryCard` を縦中央寄せしたうえで core を下へ8pxずらしており、
カード高が内容高に届かず `overflow:hidden` で % が切れていた。390px幅で **8.1pxはみ出し**。
320px・430pxでも切れていた。

上端から積む形に変え（`align-items:flex-start`、`transform:translateX(4px)`）、
`min-height:108px`、`padding:22px 4px 6px` を与えた。

**QAの穴も塞いだ。** それまで `ringContained`（1枚目の円がカード内か）しか見ておらず、
%が切れても検出できなかった。全カードの%はみ出し量を測る
`recordGlyphPercentInsideCard` を追加。修正前のCSSに戻すと320/390/430pxの3幅で落ちることを確認済み。

#### 3. 配色Gの「地」を適用（PR #18）

§56 は accent と主ボタンだけがGで、**地・紙・罫線・インクは旧配色のまま**だった。
提示値へ揃えた。

```
--bg:#fbf7f5;  --paper:#fffdfb;  --paper2:#fdf6f4;
--ink:#2f3138; --muted:#77737a;  --line:#ecdfdb;  --ring-track:#ebe3df;
```

背景のグラデーションも、桜10%＋空7%の2枚重ねから **提示どおりの空1枚**へ戻した。
`theme-color`、保存PNGの地・紙・インクも同期。旧 `#f2efe8` / `#fffdf8` / `#e5e1d8` は全廃し、
テストで再混入を止めている。

#### 4. 読みの問題で字母を伝える（PR #18）

`readingFeedbackText()` を追加し、4択・自由入力の両方で同じ文面を使う。

> 正解です。字母は「能」です。 ／ 「き」と読みます。字母は「支」です。

#### 5. 「字母」に「じぼ」のルビ（PR #18）

**ルビは行boxを押し広げるため、その見出しだけ背が高くなる。**
行の高さを上げて吸収しようとしても追いつかない（line-height 2.16でも2px残った）。
`rt` を絶対配置にして行boxの外へ出す方式にした。

```
.helpBody h3 ruby{position:relative}
.helpBody h3 rt{position:absolute;left:0;right:0;bottom:calc(100% - 2px);font-size:7px;...}
```

見出し3つとも **24.05px で完全一致**、ダイアログ全体の高さも変わらない。
ルビは漢字1字ずつに分け（`<ruby>字<rt>じ</rt></ruby><ruby>母<rt>ぼ</rt></ruby>`）、
「じ」を「字」、「ぼ」を「母」の中央へ（実測ずれ0px）。

#### 6. 縦書き（PR #18・#19）

「できあがり」の右に 横書き／縦書き のピル。`writing-mode:vertical-rl`、
縦幅300pxの固定枠、あふれは横スクロール。保存PNGも縦組みに追従する。

実装で踏んだ落とし穴が3つある。**同じことをするときは必ず確認すること。**

1. **幅を明示しないと枠が画面外まで広がる。**
   縦組みでは幅がブロック方向になり、内容なりに伸びる（実測605px）。
   `width:100%;max-width:100%` が要る。
2. **`text-orientation` の既定 `mixed` では濁点の合成文字だけが90度回転する。**
   `upright` を明示する。
3. **字ごとに送り幅が違うので、そのままでは列の字数が揃わない。**
   各字を `<span class="pvCell">` で包み、`inline-size:1.12em` で送りを固定した。
   390px幅・枠高300pxで1列7字に揃う。保存PNGの送りも同じ 1.12em。

保存PNGは当初、列ごとに天地中央で置いていたため **字数の少ない最後の列だけ宙に浮いた**。
いちばん長い列で天地を決める `columnTop` を導入し、どの列も同じ高さから始まるようにした（PR #20）。

#### 7. 一覧の50音表を母音で揃える（PR #18）

- や行 `や ・ ゆ ・ よ`（ア・ウ・オの欄）
- わ行 `わ ゐ ・ ゑ を`（ア・イ・エ・オの欄）
- 「ん」はワ行ではないので**独立した行**へ

**`KANA_ROWS` は触っていない。** あれは記録の行習熟度と出題プールの正本。
一覧の並びだけ `KANA_FILTER_ROWS` として別に持ち、空き欄は `.kanaFilterSlot` で埋める。
`check:vintage-kana` の期待行数も 10→11 へ更新した。

#### 8. 「似ている字」を並べて出す（PR #20）

実機から「注記の似ている対象が違うのでは」と指摘があった。
**フォント描画を並べて確認した結果、注記の内容は正しい。**
`U+1B052`（せ／世）は実際に現代の「を」とよく似た形で、
`U+1B11C`（を／遠）**も**「を」に似ている。だから一覧では「を」に見える字が
「せ」の欄と「を」の欄の両方に出る。これが混乱の元。

注記だけでは読者が確かめようがないので、`glyphInfoDialog` に比較対象を並べるようにした。

| 開いた字 | 並べる字 |
| --- | --- |
| せ／世（U+1B052） | 現代のかな「を」／ 変体仮名「を」（遠） |
| を／越（U+1B11A） | 現代のかな「せ」 |
| を／遠（U+1B11C） | 現代のかな「を」／ 変体仮名「せ」（世） |

`GLYPH_INFO_NOTES` を文字列から `{note, compare:[{character,kind,detail,modern}]}` へ変更。
ラベルは種別（現代のかな／変体仮名）と字を2段に分け、枠76pxに収めている。
現代の仮名は明朝、変体仮名は専用フォントで描き分ける。

#### 9. 説明ダイアログの「読み」「字母」のずれ（PR #20）

ラベル14px・値20pxを grid の既定（上端揃え）で並べていたため字の座りがずれていた。
`align-items:baseline`、ラベル右寄せ・値左寄せへ。

### 公開の記録

§52 の no-common-ancestor 手順どおり、最新 `main` 起点の公開専用ブランチへ
`vintage-kana/index.html` だけ移植して PR→マージ。**公開差分は毎回この1ファイルのみ。**

| PR | 公開ブランチ | 内容 |
| --- | --- | --- |
| #17 | `publish-vintage-kana-20260921b` | 小書き仮名・習熟度%の切れ |
| #18 | `publish-vintage-kana-20260921c` | 配色Gの地・縦書き・字母の読み上げ・ルビ・50音表 |
| #19 | `publish-vintage-kana-20260921d` | 濁音の回転・列の字数 |
| #20 | `publish-vintage-kana-20260921e` | 似ている字・上揃え・ベースライン |

`main` が別作業で進むことがあるため、**公開ブランチを作ったあと時間が空いたら
`git diff --stat origin/main HEAD` で差分が1ファイルであることを必ず確認する。**
#17のとき、main が進んだせいで他アプリの作業を全部消す差分になりかけた。
force-push は使わず `git merge origin/main` で解決した。

### 開発ブランチのコミット

```
9ed4737 似ている字のラベルで「現代のかな」「変体仮名」を名指しする
7856722 似ている字を並べて出し、縦書き書き出しを上揃えにする
ea726a7 縦書きで濁音が回転する問題と、列ごとの字数のばらつきを直す
6c60b37 配色Gの地を適用し、縦書き・字母の読み上げ・50音表の並びを直す
3ce4fe5 §58として今回の修正と公開待ちの状態を記録する
2affb7b 小書き仮名を全小文字へ広げ、習熟度カードの%が切れるのを直す
```

### 検証

各コミットで CI相当 **12項目**を全通過させている。

```
check:eol / typecheck / lint / test / data:check / build / scan:publish /
check:overflow / check:vintage-kana / check:font / check:font-assets / check:font-weight
```

`tests/unit/vintage-kana-rc25.test.ts` は **24件**。この節で追加した契約:
小書き仮名の全字対象、習熟度%がカード内、配色Gの地、読みの解説の字母、
ルビの行送り、縦書き（正立・送り・画面とPNGの一致）、50音表の母音揃え、
似ている字、ベースライン、保存PNGの上揃え。

### 残件

- NINJAL §7 の 4以降。**`observed` でおまかせ生成・出題の母集団を固定してから
  公式265件を投入する順序を崩さないこと。** 逆にすると観測0回の字体で
  実在しない字面の文章を作る。
- 混同ペア表を作り、4択の誤答候補から除外する
- 字母逆引きの単答契約を複数正解へ
- Unicode未付与28件の一覧表示（画像は同梱しない。許諾未照会）
- `SOURCES.md` の「264字体」を「293行／うちUnicode付き265」へ更新
- 国立国語研究所への字形画像の利用許諾照会

### 後片付け

- `D:\dev\koten` の `.claude/launch.json` はプレビュー用サーバ設定を足して使っていた。
  セッション終了時に元へ戻してある。
- `D:\dev\koten-publish-tmp` は公開ブランチ組み立て用の一時worktree。
  不要なら `git worktree remove D:\dev\koten-publish-tmp`（ブランチは残る）。
- マージ済みの公開ブランチ `publish-vintage-kana-20260921b`〜`e` は残したままにしてある。
  他チームのブランチと取り違えないよう、消すときは名前を確認すること。

### 次回再開

1. `vintage-kana/HANDOFF.md` §56〜§59 を読む。§58の「公開待ち」は解消済み。
2. `origin/vintage-kana-main` を開発正本としてブランチを切る。
3. まず公開版を実機確認する。
4. NINJAL残件へ進む場合は、監査文書の3層構造と `observed` 境界を守る。

再開指示:
`vintage-kana-main HANDOFF §56〜§59から再開。配色Gの地・縦書き・似ている字・50音表・小書き仮名は実装/CI/Pages公開済み（PR #17〜#20）。SSH鍵でpushは通るがghは無いのでPR作成のみ手作業。次はNINJAL残件で、observedで母集団を固定してから公式265件を投入する。`

## 60. 2026-09-22〜23 UI改修・NINJAL公式全件統合・セッション区切り

§59 の続き。この節の内容のうち、**公開済みは PR #23 まで**。
NINJAL公式全件の統合と改行修正は **公開ブランチを用意した状態で止まっている**（末尾参照）。

### ブランチ

- 開発正本: `origin/vintage-kana-main`
- ローカル作業ブランチ: `vintage-kana-resume-20260921`（worktree `D:\dev\koten-vintage-kana`）
- ローカルの `vintage-kana-main` ref は `91ae986` のまま取り残してある。**使わないこと。**

### この節で実装したこと

#### 1. 結果画面を「今回確認した字」にした（PR #23で公開済み）

点数しか出しておらず何を確認したのか残らなかった。
そのセットで出た5字を変体仮名だけで並べ、押すと字形解説ダイアログが開く。

正誤の印は **正解 ◎ / 誤答 ✓**。日本の採点では ✓ が誤答を指すことがあり、
✕ と意味が重なるため分けた。誤答だけ地と枠に色を付け、正解は静かな印だけにしてある。
両方を同じ強さで飾ると直すべき字が埋もれるため。
色だけに頼らないよう記号でも区別し、`aria-label` にも正誤を載せている。

1問ごとの正誤は `quizSet.results` に持つ。
`quizSet.glyphs` は出題の重複除けに使っているので触っていない。

#### 2. おまかせで入力のたびに既出の字が変わる問題（PR #23で公開済み）

入力イベントのたびに `randomizeCompose()` で全字を引き直していた。
一文字打つたび・改行するたびに既出の字まで変わり、
暗号文のように書き進める使い方ができなかった。

入力時は `renderCompose()` を呼ぶ。こちらは未割当の字にだけ割り当てるので既出は残る。
全部引き直すのは「もう一度」だけ。
...
| 一字ずつ打つ | 既出の字は変わらない |
| 改行する | 既出の字は変わらない |
| 同じ仮名を複数回 | 常に同じ字体（一貫した置換になる） |
| 「もう一度」 | 全字を引き直す |

#### 3. おまかせの抽選を頻度で重み付け（PR #23で公開済み）

**これは265件投入の前提条件だったので先に入れた。**

それまで同じ仮名の字体から一様に選んでいた。3848回の字体と1回の字体が同じ確率で出る。
候補が266件へ増えるとこの偏りはさらに大きくなる。

生の回数を重みにすると「る」で137倍の開きがあり事実上1種類しか出なくなるため、
**平方根で均して約12倍に収めた**。`AUTO_WEIGHT_FLOOR=2` を足して下限を持たせている。

**観測0回を「使われていない」と扱ってはいけない。**
観測0回は「この15資料には出てこなかった」という意味にすぎず、
国立国語研究所の一覧に載っていること自体が典拠である。
監査文書 §6 の `observed === true` で母集団を絞る案は **ユーザー判断で却下**されている。
頻度は重みとしてのみ使い、除外はしない。
...
#### 4. 保存画像のクレジット（PR #23で公開済み）

細罫＋明朝の署名が紙の内側にあり本文と同じ面で主張していた。
紙の外（下の余白）へ移し、罫をやめ、タイトルと同じ丸ゴシックの薄い一行にした。
表記は「変体仮名メーカー © 2026」。描画前にフォント読み込みを待つ処理も入れてある。

#### 5. 「字母」のルビ（PR #23で公開済み）

一度ルビ（`<ruby><rt>`）で実装したが、**ルビは行boxを押し広げてその見出しだけ背が高くなる**。
行の高さを上げても追いつかず、絶対配置で行boxの外へ逃がす必要があった。
最終的にユーザー判断で **括弧書き「字母（じぼ）とは？」** へ戻し、関連CSSも撤去した。

#### 6. できあがりに改行が反映されない（**未公開**）

各字を `<span class="pvCell">` で包む際、改行も包んでいた。
HTMLの既定では枠の中の改行は空白に潰れるため、画面では改行が消え、
保存画像だけ改行されるという食い違いが起きていた。

改行は字ではないので包まず `<br>` として出す。横書き・縦書きとも反映される。

#### 7. NINJAL公式全件の統合（**未公開**）

ChatGPT Thinking へ切り出して実施。指示書は `NINJAL_INTEGRATION_TASK_2026-09-23.md`。

結果を**ローカルで独立に検証済み**:

| 検証項目 | 結果 |
| --- | --- |
| `ui-glyph-master.json` | 266件（`glyphCount` も266） |
| `FALLBACK_GLYPHS` | 266件・JSONとして完全同値 |
| 内訳 | 公式265 ＋ 公式外 `U+1B10C` 1件 |
| `observed>0` / `==0` | 138 / 128 |
| 符号位置の重複 | 0 |
| `renderability` | 全件 `unicode-text` |
| カタログ | 293件のまま保持 |
| `index.html` の `FALLBACK_GLYPHS` 以外 | **一字も変わっていない**（CSS・`autoWeight`・`pickWeighted`・`pickAutomatic`・改行修正すべて無傷） |
| CI相当12項目 | 全通過（ローカルで再実行して確認） |

指示書に「127件追加」と書いたのは**こちらの誤り**だった。
作業前138件の内訳は 公式137 ＋ 公式外1 であり、正しくは128件追加。
ChatGPT が実データと突合して訂正してきたもので、最終件数266は指示どおり。

266件での記録画面を390px幅で実測:
...
**`check:vintage-kana` は落ちなかったのでCSS調整は不要だった。**

### 公開状態

- **PR #23 までは公開済み。** `main` の `vintage-kana/index.html` blob = `fa12e76`
- **6・7は未公開。** 公開ブランチは用意済み。

```
branch: publish-vintage-kana-20260923
commit: 0f6c3a3
差分: vintage-kana/index.html と vintage-kana/data/ui-glyph-master.json の2ファイル
公開後の想定 index.html blob: a180ea3855b47f231ea0c3f9e487e6b549166e58
```

PR作成URL: `https://github.com/yama-books/koten/pull/new/publish-vintage-kana-20260923`

**今回はデータファイルも公開対象**である点に注意。
アプリは実行時に `./data/ui-glyph-master.json` を取りに行くので、
`index.html` だけ公開すると `FALLBACK_GLYPHS` の266件で動きはするが、
データ取得経路だけ138件のままになり両者が食い違う。

### 環境まわり

- `git push` は **SSH鍵** で通る。remote は `git@github.com:yama-books/koten.git`。
- **`gh` CLI は入っていない。** PR作成だけは手作業。
- `main` は他アプリ（conj 等）の作業で頻繁に進む。
  **公開ブランチを作った後に時間が空いたら、PRを出す前に必ず
  `git diff --stat origin/main HEAD` で差分が想定ファイルだけか確認すること。**
  この節でも、放置した公開ブランチが conj の変更を巻き戻す差分になっていた。
  force-push は使わず `git merge origin/main` で解消する。

### 残件

1. **公開**（上記 publish ブランチのPR→マージ→Pages確認）
2. **混同ペア表と習熟度による出し分け**
   - 低習熟のうちは混同ペアを4択の誤答候補から**除外**
   - 高習熟に達したら逆に**優先して並べる**（見分けの訓練にする）
   - 音価を横断して誤答を引くと「せ／世」と「を／遠」のような組が同じ4択に並ぶ
3. **字母逆引きの単答契約を複数正解へ**（同じ字母に複数字体が50組115行ある）
4. Unicode未付与28件 — **扱わない方針**。一覧にも出さない。
   字形の提示手段が公式サイトの画像だけで再配布条件が未確認のため。
   リンクだけ張る案も「不十分感を与える」としてユーザー判断で見送り。
   カタログ293件は監査用に保持する。

### 漢字書体について

漢字かな交じりで書いたときの漢字書体を検討した。
現状は `.preview` のフォントスタックにより **Klee One** で出ている。

候補6種（Klee One / Shippori Mincho / Zen Old Mincho / Kaisei HarunoUmi /
Yuji Syuku / Noto Serif JP）を同じ文・同じ寸法で並べた比較ページを作成:
`https://claude.ai/artifact/82UHgY5tWekH3eKYSVLoJW`

**結論: 今回は変更しない（ユーザー判断）。**

Adobe Fonts の HOT-白舟行書教漢 も検討したが、次の理由で見送り。

- 配信が Adobe のキット経由のみで自己ホスト不可。OFLではない
- Creative Cloud の契約が切れると**予告なくフォールバックに落ちる**
- より本質的に、**漢字まで行書にすると初学者の足場が消える**。
  このアプリは変体仮名を読めるようにするものなので、漢字は読み手の足場として働いている

もし将来入れるなら、既定にせず選べるようにするか、画面は読みやすい書体のまま
**保存画像だけ行書**にするのが用途と合う。

### 使っているフォント（確認済み）

記事で紹介されていた Google の変体仮名フォントは、**すでにこのアプリで使っている**。
`Noto Serif Hentaigana` がそれで、Unicode登録済みの変体仮名286文字（U+1B001〜U+1B11E）を
カバーする。ライセンスは SIL Open Font License 1.1。

**Unicode未付与28件はこのフォントでも表示できない。**
フォントは符号位置に字形を対応させる仕組みなので、符号位置が無い字は呼び出す手段がない。
私用領域に独自の符号位置を割り当てた派生フォントを作れば技術的には可能で、
OFL-1.1 は派生フォントの作成を許している。
ただし**字形の出所**（NINJALの画像）は別問題として残る。

### 次回再開

1. `vintage-kana/HANDOFF.md` §56〜§60 を読む。
2. `origin/vintage-kana-main` を開発正本とする。
3. まず `publish-vintage-kana-20260923` を公開し、実機で266件を確認する。
4. そのあと残件2・3（混同ペアと複数正解）へ進む。

再開指示:
`vintage-kana-main HANDOFF §56〜§60から再開。NINJAL公式266件の統合と改行修正は実装・CI通過済みだが未公開。publish-vintage-kana-20260923 のPRを出して公開するところから。次は混同ペアの習熟度別出し分けと字母逆引きの複数正解。observedで母集団を絞る案は却下済み、頻度は重みとしてのみ使う。`

## 61. 2026-09-23 NINJAL公式266件・改行修正の公開完了

§60 の続き。§60 の6・7（改行修正とNINJAL公式全件統合）を公開した。

### 公開

| 項目 | 値 |
| --- | --- |
| PR | #27 `publish-vintage-kana-20260923` → `main`（2026-09-23 06:02 UTC マージ） |
| マージコミット | `3dd18d7` |
| Deploy Pages / CI（`main`） | 両方成功 |

公開URL `https://yama-books.github.io/koten/vintage-kana/` から直接取得して、
`main` のファイルと **SHA-1 が一致** することを確認した。

| ファイル | 一致 |
| --- | --- |
| `vintage-kana/index.html` | 一致 |
| `vintage-kana/data/ui-glyph-master.json` | 一致（公開版に `ninjal_id` 266件） |

改行修正（`23685c6`）は公開中の `index.html` に含まれている。
**画面上の見え方（改行・266件の記録画面）は実機でまだ見ていない。** 次に触るときに確認する。

### §60 の記述の訂正

- 「`gh` CLI は入っていない」は **この環境では誤り**。`gh` は使えて、PR作成・Actionsの監視もできる。
  手作業が要るのは `gh` が無い端末からのときだけ。

### 残件（§60 から繰り越し）

1. **混同ペア表** — データ作成を ChatGPT Thinking へ切り出した。
   指示書: `vintage-kana/CONFUSION_PAIRS_TASK_2026-09-23.md`
   **データだけ**の作業で、`index.html` には触らせない。
2. **混同ペアの習熟度別出し分け**（ローカル）— 1 のデータが揃ってから。
   - 低習熟: 混同ペアを4択の誤答候補から除外
   - 高習熟: 逆に優先して並べる
   - `reverseJiboChoices()` は音価を横断し、**標準ひらがな（`isStandard`）も選択肢に入れる**。
     したがって表には「変体仮名 × 変体仮名」だけでなく「変体仮名 × 標準ひらがな」の組も要る
     （例: `U+1B052` せ／世 と 標準の「を」）。
3. **字母逆引きの複数正解**（ローカル）— 同じ字母に複数字体がある。
   現状 `reverseJiboChoices()` は同字母の字を選択肢から外すことで単答を保っている。
4. Unicode未付与28件 — 扱わない方針のまま。

### 次回再開

1. §60・§61 を読む。
2. ChatGPT から混同ペア表が戻っていれば、`origin/vintage-kana-main` を取り込んで独立に検証する。
3. 検証が通れば残件2（出し分け）、続けて残件3（複数正解）。
...
## 63. 2026-09-24 出題とおまかせの頻度の扱い

§62 の続き。どちらも `vintage-kana-main` までで、`main` への公開はまだ。

### おまかせの初期表示を最頻の字体に（`48e8e0a`、Sonnet 実装）

- 未割当の字の初期表示は `pickDefault()`。その仮名の中で最も頻度の高い字体を選ぶ（`compareGlyphBrowseOrder` の先頭）。
- 「もう一度」（`randomizeCompose()`）はこれまでどおり `pickAutomatic()` による重み付きランダム。

### 出題を頻度で重み付けし、習熟に応じて珍しい字を増やす

それまで出題は266字からの**一様ランダム**で、観測0回の128字が出題の約48%を占めていた。
ユーザー判断で、次の仕組みにした。

```
重み = 出現回数 ^ 指数 + AUTO_WEIGHT_FLOOR(2)
指数 = QUIZ_WEIGHT_EXPONENT_MAX(0.5) × (1 − 進み具合)
進み具合 = Σ autoWeight(字) × 習熟度(字)/100 ÷ Σ autoWeight(字)   … 0〜1
```

- 進み具合は、**よく出る字ほど重く数えた平均習熟度**。よく出る字を覚えるほど指数が下がり、珍しい字が出やすくなる。
- 指数 0.5 は おまかせの `autoWeight`（平方根）と同じ。指数 0 で全字一様。
- 観測0回の字も下限の重みで必ず出る。**除外はしない**（`observed` で絞る案は却下済み）。
- 実装は `pickQuizWeighted()` / `quizWeight()` / `quizFrequencyProgress()`（`newQuiz()` の直前）。
  おまかせ側の `autoWeight` / `pickWeighted` / `pickAutomatic` は変えていない。

実データでの観測0回の字の出題割合（ブラウザで2万回抽選して確認。計算値と一致）:

| 進み具合 | 指数 | 観測0回の割合 | 最多の字1つの割合 |
| --- | --- | --- | --- |
| 0 | 0.5 | 11.5% | 2.83% |
| 0.25 | 0.375 | 18.7% | 1.75% |
| 0.5 | 0.25 | 26.6% | 1.02% |
| 0.75 | 0.125 | 33.6% | 0.63% |
| 1 | 0 | 48.7% | 0.38% |

字母逆引き（習熟65%以上の字だけの出題）にも同じ重みがかかる。

確認: `npm run test:node` 791 pass / 0 fail、`check:vintage-kana` findings 0、ブラウザでコンソールエラーなし。

### 残件

§62 の残件1〜3のまま。加えて、一連の変更（§62・§63）の `main` への公開。

再開指示:
`vintage-kana-main HANDOFF §62〜§63から再開。混同ペアの出し分け・おまかせ初期表示の最頻化・出題の頻度重み付け（習熟に応じて珍しい字を逓増）はvintage-kana-mainにpush済みでmainへの公開はまだ。残件は字母逆引きの複数正解、GLYPH_INFO_NOTESへの「似ている字」表示、U+1B052の似ている字の食い違い（U+1B11B対U+1B11C）の監査。`

## 64. 2026-09-24 字母逆引きは選びなおしで単答を保つ・公開

### 字母逆引き: 複数正解にせず、選びなおす

ユーザー判断で、同じ字母に複数の字体があっても **複数正解にはせず、正解が重複したら選びなおす** 方式にした。

- 候補の組み立ては `drawReverseJiboChoices()`（旧 `reverseJiboChoices()` の中身そのまま。同字母の除外・混同ペアの出し分けも含む）。
- `reverseJiboChoices()` は組み立てたあとに `isSingleAnswerReverseJibo()` で検査し、
  「4つ・正解を含む・同じ字母の字がちょうど1つ・字の重複なし」を満たさなければ最大 `REVERSE_JIBO_MAX_REDRAWS=20` 回選びなおす。
  それでも駄目なのはデータ不整合だけなので、警告を出して同字母の誤答を落として返す。
- 字母の比較は `jiboKey()`（NFC＋空白除去）。

**除外は字母の文字列一致に頼っている**ため、異体字の書き分け（与/與、礼/禮 など）があると正解が2つになる。
現状は標準ひらがな（`STANDARD_HIRAGANA_JIBO`）も変体仮名も旧字体で揃っていて問題ない。
これを `tests/unit/vintage-kana-rc25.test.ts` に固定した。
例外: 「余」と「餘」は NINJAL 公式一覧で別の字母（`380020010` / `380040010`）なので、別答として扱う。

ブラウザ確認: 全266字×30回＝7,980回の組み立てで、正解がちょうど1つでないものは0件。
1回目にわざと同字母の字を混ぜた場合、2回目の組み立てで正解1つの組に直ることも確認した。

### 否定形の出題（「次のうち『あ』ではないものはどれ？」）— 未実装・検討メモ

ユーザーから「実装負荷が少ないなら」との提案。**今回は見送った。** 理由:

- **字母版（「字母が〇でないもの」）は成り立たない字母が多い。** 同じ字母の字体が3つ以上あるのは5字母だけ（1つ169・2つ41・3つ5）。
- **読み版（「『あ』ではないもの」）は成り立つ**（字体が3つ以上ある仮名が45/46）。
  ただし4字すべてが判断対象になるため、**習熟度をどの字に記録するか**（外れの1字だけか、4字全部か）の設計が要る。
  いまの記録は1問1字（`recordLearningAttempt()` は `quizEntry` だけ）で、ここを広げると記録画面・日次上限にも波及する。
- 出す時期（どの習熟度で、どのモードに混ぜるか）もユーザー判断が要る。

案: 読み版を、その仮名の字体の平均習熟度が一定以上のときに「読み」モードへ低頻度で混ぜ、正誤は外れの1字にだけ記録する。
...
## 66. 2026-09-24 §62〜§65 の公開（PR #29）

| 項目 | 値 |
| --- | --- |
| PR | #29 `publish-vintage-kana-20260924b` → `main`（マージコミット `032a6a9`） |
| 元にした開発版 | `vintage-kana-main` `2109422` |
| 公開したファイル | `index.html` / `data/ui-glyph-master.json` / `data/glyph-confusion-pairs.json` |
| Deploy Pages / CI（`main`） | 両方成功 |

公開URLから取得した3ファイルは `main` と SHA-1 一致。公開ページで 266字・混同ペア読み込み・案内表示・コンソールエラーなしを確認。
先に作った `publish-vintage-kana-20260924` は #29 に置き換えたので使わない。

公開前にローカルで行った確認（Sonnet の作業環境では実施できなかった分）:

- 否定形出題: 習熟60未満で300回中0回、60以上で500回中100回（20%）。100回とも「4択・変体仮名のみ・Kでない字がちょうど1つ・それが quizEntry・混同相手なし」。
  正解・不正解のクリックで表示と記録（`questionType:"reading-negative"`、外れの1字に1件）を確認。
- ホーム画面案内: ×を右上に固定し、文は `<wbr>`＋`keep-all` で文節だけで折り返すよう修正（`2109422`）。320px とPC幅で3種の文面を確認。

### 環境メモ

- PR は **`yama-books` アカウント** で作る。別アカウントでは「must be a collaborator」で作れない（iPhone で発生）。
- `gh` のトークンは PR 作成の権限がない（`createPullRequest` が拒否される）。PR 作成とマージはブラウザで行う。Actions の監視は `gh` でできる。

### 残件

1. ホーム画面案内: 追加ボタンの確認をキャンセルしても案内が閉じたままになる（Android/PC の Chrome）。
2. ホーム画面案内・否定形出題の iPhone 実機での見え方。
...
## 69. 2026-09-24 ホーム画面案内を1行に・行ボタン「ん」の幅

- ホーム画面案内: 文字を 11px → 10px、左右の余白を 14px → 12px にし、390px 幅で文を1行に収めた。

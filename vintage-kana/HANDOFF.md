...
未完了:
- 0件の明示的補完と比率計算は派生データで完了。生データは非ゼロ転記のまま保持
...
1. `sources.json` の年代・ジャンル・所蔵・利用条件を確定する
2. 分布差が大きい字体を抽出し、実文脈採取の優先順位を作る
3. 高頻度字体・特徴的字体から原資料の実文脈を採取する
4. `attested-examples.json` に保存する
5. 字母・字体クイズ候補を生成する
6. 短い `attested` 読解教材を生成する
7. 人間が典拠と表示を確認する
8. 「書いてみる」候補表示を実例データと接続する

...
- **確認状態**の `pdf_visual_checked_candidate` は「PDF画像との照合は済んだが、**台帳としての人手承認はまだ**」という意味である。行ごとの確認状態であって、反映済みかどうかを表さない。
- **正本MDへの反映は済んでいる**（2026-09-10）。`百人一首_本文・作者_一次データ.md` は
  本表の「原本表記（PDF）」を表示の正本として持ち、「許容解答（現MD）」は入力判定で正答として受理する。
  **この一覧を編集したら、同じコミットでデータを生成し直すこと。** 件数（127件・75首）を釘付けにした
  試験があるので、一覧だけを直すと落ちる。

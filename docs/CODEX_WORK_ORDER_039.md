...
## 9. 差し戻し 1（2026-09-02・第20回。**親担当が破壊試験を自分で回した結果**）

**実装は正しい。落ちたのは試験である。** 変更 6 ファイルはそのまま残し、**試験だけを直すこと。**
`main.tsx`・`Home.tsx`・`Result.tsx` の**実装を書き換える必要は無い**（親担当が確認済み）。

### 9.1 親担当が実測した結果（**報告からの転記ではない**）

**通ったもの**: A-1〜A-6・A-9〜A-20（`test:node` 322/322、`test:screen` 75/75＝基準 +14、
`scan:publish` 751/違反 0、build・typecheck・lint・data:check すべて 0、
凍結 6 ＋ 一次資料 5 のハッシュ **11/11 一致**、`Result.tsx` は `1	1`、§5.0 の grep **6 本とも 0 件**、
A-13 の grep は着手前の `main.tsx` で 1 件出ることも確認済み＝**常に 0 を返す壊れた検査ではない**）。

**A-21・A-22 は合格**——親担当が C-11・C-12 を実測した:

| # | 壊した箇所 | 結果 |
|---|---|---|
| C-11 | `main.tsx` の `result-save-failure` の一行を削除 | **`main: 保存失敗でも結果と失敗の一行を表示する` だけが赤**（1 failed / 74 passed） |
| C-12 | `main.tsx` の `result-loading` の行を削除 | **`main: 結果の読み込み中は完了前に文言を表示する` だけが赤**（1 failed / 74 passed） |
| C-2 | 新規開始で範囲の全首を渡す | **`main-wiring: 21首の新規開始は最初の20首だけを出す` だけが赤**（1 failed / 74 passed） |

**発注037 で見つかった 2 つの穴は塞がった。C-2（依頼者の目的）も守られている。**

### 9.2 **不合格 4 件。すべて「壊しても 75/75 が緑のまま」である**

| # | 壊し方（親担当が実際に行った） | 結果 | アプリ機能上の帰結 |
|---|---|---|---|
| **C-6** | `main.tsx` の `input.session.seed ?? ''` を `createSeed(Math.random)` に変える | **75/75 緑** | 復元しても**出題順が別物になる**。「同じ回の続き」（裁定 1）が壊れても誰も気づかない |
| **C-7** | `main.tsx` の `cardNumbers: cards` を範囲全体に変える（2 箇所） | **75/75 緑** | **「まちがえた歌だけをもう一度」を押すと範囲全体が出題される**——ボタンの文言が嘘になる（裁定 7・D-53 の中心） |
| **C-8** | `Home.tsx` の「復元しない」に `saveSession({ ...session, completed: true })` を足す | **75/75 緑** | **「復元しない」が保存を書き換える**。終わっていない学習が終わった扱いになり、二度と誘われない（裁定 3・憲章 §1 の記録の嘘） |
| **C-9** | `Home.tsx` の `あと{plan.remainingInRange}首` を `あと{plan.cardNumbers.length}首` に変える | **75/75 緑** | **「あと○首」がまとまり内の数になる**。範囲 1〜100 の残り 80 首でも「あと20首」と出て、学習者が終わりを見誤る |

### 9.3 原因（**それぞれ型が違う。まとめて直さないこと**）

- **C-6・C-7 は「試験が 1 本も無い」型。** 再確認（`onRetryWeak`）と復元後の seed を、**`main.tsx` を通して**見る試験が存在しない。
  `result.test.tsx` の 5 本は `Result` が親へ `[3, 5]` を渡すことしか見ておらず（`onRetryWeak` はスタブ）、
  **その先で `main.tsx` が何をするか＝裁定 6 の配線そのものを誰も検査していない。**
- **C-9 は fixture の退化型。** `restore: 誘いに範囲全体の残りとまとまりの進捗を出す` は存在するが、
  fixture が範囲 1〜21・確認済み 20 首のため **`remainingInRange` が 1、まとまりも `[21]` の 1 件**で、
  **2 つの実装が同じ「あと1首」を出す。** §5.2 の 3（確認済みをまとまりごとに偏らせる）に反する。
- **C-8 は試験名と assert の不一致型（D-49）。** `restore: 復元しないは保存を書き換えず同じ起動では畳む` は
  **`port` を 1 度も見ていない**（畳んだことと `onResume` 未呼び出しだけを見ている）。§5.2 の 6 に反する。

### 9.4 やること（**これだけ。実装には触らない**）

1. **`tests/screen/main-wiring.test.tsx` に再確認の配線試験を足す**（C-7 用）。
   結果画面から「まちがえた歌だけをもう一度」を押し、**出題された首が渡した番号だけであること**を assert する。
   §5.2 の 4・5 を守ること——「空でないこと」を先に見て、「どの首が出るか」まで書く
2. **復元後の seed を見る試験を足す**（C-6 用）。**`main.tsx` を通して**、保存された seed で並んだ順序が再現されることを見る。
   `order: 'random'` を使うこと——**`order: 'number'` では seed を変えても順序が変わらず、試験が効かない**
3. **`restore.test.tsx` の fixture を、2 実装を区別できる値に直す**（C-9 用）。
   範囲を広げ確認済みを偏らせて、**`remainingInRange` と まとまりの首数が別の数になる**ようにする（例: 残り 21・まとまり 20）
4. **`restore: 復元しない…` に `saveSession` が呼ばれていないことの assert を足す**（C-8 用）。
   `createMemoryPort()` を包んで `saveSession` の呼び出しを記録する
5. **C-6〜C-10 を自分で回し、出力を貼る。** C-10（`questions={[]}` に差し替えて緑のまま残る試験が無いこと）は**未報告である**

**完了報告には §7 の 8 項目に加えて、C-6・C-7・C-8・C-9 の破壊試験の出力（赤くなった試験名と件数）を必ず貼ること。**
**「赤くなるはず」ではなく、実際に回した出力を貼ること。**

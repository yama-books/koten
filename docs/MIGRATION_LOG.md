...
| **P-3** | **テスト公開の範囲では、押すたびの確認は要らない**（2026-09-05・依頼者裁定 → **D-75**）。**実機確認のために公開するのが目的であり、URL は未告知である。** ただし**§7.3 の差分確認は毎回行う**——**押す前に守っているのは確認の一手ではなく差分の点検である。** **告知後、または許可リストの外へ出るものが差分に現れたときは、この裁定は効かない。止まって報告する** |

...
- **基準線は commit `de50841`。実装はその上の1コミットとして載っている。**本書で基準線を書くのはここ1か所だけである。
...
**作業ツリーは clean であること**（`git status --short` が空）。実装は基準線の上の**1コミット**である。
...
**`packages/shared/src/release-notes.ts` は §18 の変更境界に入っていない。** 工程3 本文が定義の削除を求めているため残してあるが、**境界違反として上げるかどうかは裁定である。**
...
| `npm test`（画面） | **24 files / 260件** すべて緑 |
| `lint` / `typecheck` | 出力なし・exit 0 |
| `data:check` | exit 0 |
| `scan:publish` | 走査 **758** 件・違反 0 |
| `check:eol` | 走査 **1110** 件・違反 0 |
| `check:font-weight` | 走査画面 4 件・テキスト要素 **51** 件・違反 0 |
| `check:overflow` | 合計 **1800** 件・全件合格・**全群 違反0**・**exit 0** |
...
| 5 | **工程5 の探索結果は8か所**（該当2・修正済み、境界例1種）。一覧は `docs/HANDOFF.md` と実装報告にある。**`RecordTransfer.tsx` の確認ダイアログを「該当」と数えるなら3件になり、§20 の「止まる」条件に触れる** |
...
現在の保存済みプロジェクト（`C:/Users/user/AI開発/koten`）で検収してください。**実装は基準線の上の1コミットです。**

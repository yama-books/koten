...
**裁定 D-07（2026-08-30 確定。旧 ※要確認 ①）**: 静的アセットの置き場を次のとおり固定する。

| 対象 | 置き場 | 各公開単位への渡し方 |
|---|---|---|
| `404.html` | **公開単位ごとに複製**（`packages/{hyakunin,kanazukai}/public/404.html`） | 複製する。内容は数行の定型で、base path が異なるため共有できない |
| フォント WOFF2 | **`packages/{hyakunin,kanazukai}/public/fonts/` に実体を置き、両方とも追跡する**（2026-09-01 に実態へ合わせた。下記） | 複製する。コピー工程も共有正本も作らない |
| feedback 画像 | **リポジトリ直下 `assets/feedback/` のまま動かさない** | `docs/PUBLISH_MANIFEST.md` §5 がこの位置を許可リストに載せているため移動しない |

`404.html` は base path 依存で共有できず、複製のほうが安い。
`public/` を Vite の `publicDir` として共有することはできない（`publicDir` は 1 パッケージに 1 つしか指定できない）。

> **⚠ フォントの行は、当初の記述が実装されなかったため 2026-09-01 に是正した。**
>
> 当初は「`packages/shared/assets/fonts/` を正本とし、各公開単位の `prebuild` がコピーする
> （コピー先は `.gitignore` する）。理由: WOFF2 の実体を git に二重で持たない」と書いていた。
> **実測（2026-09-01）では、`packages/shared/assets/` は存在せず、`prebuild` スクリプトも 1 つも無く、
> `packages/{hyakunin,kanazukai}/public/fonts/` に 371 ファイルずつが追跡されている。**
> `tools/font-assets-check/index.ts` の `fontsDir()` もこの 2 箇所を正としている。
>
> **新たな裁定は不要である。裁定 D-12（2026-08-31）が既にこの重複を追認している**
> （`dist` は 8.8MB / 8.7MB で Pages の上限 1GB に対し桁が 2 つ違う。共有配信は複雑さだけが増える）。
> **記述を実態へ合わせただけである。** 詳細は ADR-0004 追補 D-07。
>
...
├─ index.html
├─ public/                         404.html と fonts/（配置は裁定 D-07・D-12 で確定）
├─ src/
...
※要確認 ②（**保留。P1 を妨げない**）: `tools/build-data/` は現状は百人一首側のパイプライン（`parse-poems` 等）のみを列挙している。仮名遣いツールの一次データ（`仮名遣い規則_一次データ.md`・`仮名遣い語彙_一次データ.md`）に対応するパイプライン構成は、ADR-0004・先方設計書のいずれにも具体的な記載がない。

**2026-08-30 時点で仮名遣い側の一次データはまだ存在しない。** `docs/KANAZUKAI_HANDOFF.md` §5 のとおり、`仮名遣い規則_一次データ.md` の作成が同ツールの P0 であり、担当は Claude Opus 5（high）である。**データができるまでこの論点は決められない。** 決めるのはその後でよい。

> **状態の更新（2026-09-01）。** 一次データは 2026-08-31 に完成し、2026-09-01 に依頼者が承認した。
> **しかし本論点は依然として決めない。裁定 D-21 により、仮名遣いツール固有の論点は保留である。**
> **発注024 では「`tools/build-data/kanazukai/` を作らない」と明記した**（作れば S-C）。
> これは仮名遣い実装計画 §9 の提案 1 にあたり、**再開は百人一首ツールの完成後**である。
> **未決ではなく保留である。決める時期が定まっているので、ここで停止しないこと。**

※要確認 ③（**保留。P1 を妨げない**）: `tools/overflow-check/` は「100 首 × 4 幅」という百人一首前提の検査として作られている。仮名遣いツール（横書き単語モード）に同種の機械あふれ検査が要るか、要るとすればどう構成するかは未確認。~~**P3 で D-04（書体）と同時に決める。**~~

> **状態の更新（2026-09-01）。** D-04（書体）は 2026-08-30 に確定し、P3 は完了した。
> **しかし本論点は決めていない。裁定 D-21 により保留である**（仮名遣いツール固有であり、百人一首側の公開を妨げない）。
> `check:overflow` は現在 hyakunin の 1200 件（100 首 × 読み 3 × 4 幅）を検査して合格している。
> **再開は百人一首ツールの完成後。未決ではなく保留である。**

**裁定 D-08（2026-08-30 確定。旧 ※要確認 ④）**: `load.ts` は**共有層に置く**。

- `packages/shared/src/data/load.ts` — 生成 JSON の取得、実行時スキーマ検査、失敗時に ErrorScreen へ倒す動作。**製品に依存しない。**
- `packages/{hyakunin,kanazukai}/src/data/schema.ts` — 製品固有のスキーマ記述。これを `load.ts` へ渡す。

理由: 「生成物が壊れていたら ErrorScreen」は両製品に共通の不変条件である。二重に実装すると、どちらか一方の検査が緩んだときに気づけない。憲章 §1 の優先順 3（表示上の誠実さ）に直結するため、緩む経路を作らない。
...
| P6 出題生成と選題 | **完了・commit 済み** | 発注026・027・028・029 を検収（`review/*.yaml` の人確認 H-04・H-05 と併せて開いた） |
| P7 学習画面 | **完了・commit 済み** | 発注030（純関数）・031・032（画面）・033・035 を検収。`ui/screens/{Home,RangePicker,Session}.tsx` 実在 |
| P8 結果・おすすめ・要確認再試行 | **完了・commit 済み**（2026-09-03 `1f3f4ca`） | 034（P8-A 集計）・036（P8-A2）・037（P8-B 結果画面）・038（P8-C 再開計画）・039（P8-D 再開の配線）・**040（P8-E History とメーター）** を検収。`ui/screens/{Result,History}.tsx`・`shared/src/ui/components/MasteryMeter.tsx`・`domain/{result,resume,history}.ts` 実在。**040 で §8.6 の「％・メーター・5 色を同時に」が実際に満たされた**——`--mastery-*` と `--color-coral` が `tokens.css` に無く 037 以降ずっと 5 色が描画されていなかった（`tests/unit/css-tokens.test.ts` が再発を止める） |
| P9 以降 | 未着手 | **P9 は裁定 D-02（統計・報告の受け口）が未決で、H-05（Firebase プロジェクト作成・予算アラート・App Check 登録）も人の作業である。P10 は P9 依存**（P10-A の書き込み口だけは発注031 で先出し済み） |
...
#### P7 の分割と裁定（2026-09-01・第14回。**後続はこれを前提にすること**）

**P7 を 2 本の発注へ割った。** 上のブロックは正本のままで、変更していない。

| 発注 | 範囲 | 出し方 |
|---|---|---|
| **030（P7-A）** | `domain/{entry,flow,record,ports}.ts` ＋ `data/question-schema.ts` ＋ `mastery/rules.v1.ts` の 1 行。**すべて純関数。`.tsx` を 1 本も作らない** | **発注031 と並行してよい** |
| **032（P7-B）** | `ui/` 以下の `.tsx` ＋ `main.tsx` ＋ `styles.css`（追加のみ）と `tests/screen/` | **030 の検収後に単独で出す** |

**分けた理由は 2 つある。**

1. **画面に書かれた判断は `npm run test:node` の 200 件超で守られない。**
   `jsdom` の画面試験でしか触れなくなる。**守りが薄い場所へ重要な判断を置かない。**
2. **画面試験の件数が 032 の受入条件の中心である。** 並行発注があると赤緑が判定材料にならない（§9 手順 0）。

**確定した裁定は D-28〜D-37 の 10 件である**（`docs/HANDOFF.md` §4.1 と ADR-0004 追補に全文）。要点だけ再掲する。

- **D-28**: **問 0 件は正常状態である**（`questions.*.json` は現に `[]`、`approved` は全台帳 0）。
  `view` 以外の 4 入口は**縮退**する。**`poems.json` から問を合成する経路を作らない。**
  画面は「準備中」と正直に書き、**「エラー」「失敗」と書かない。**
- **D-29**: 部分正解は**歴史的仮名遣い・漢字・増分の 3 点を必ず返し、丸印もチェック印も使わない**。
  **§8.5 の表に部分正解の行が無かったのを、ここで補った。**
- **D-30**: `exam` の自己採点は **○ を 2 つに割る**（漢字・歴史的仮名遣いで書けた／現代仮名遣いで書けた）。
  △ と × は据え置く。
- **D-31**: ヒント利用は「その問を表示している間に、ルビなし以外の読み表示が有効だったか」。
  **切替の時点を問わない**（§6 の「直接関係するかを問わず」）。
- **D-32**: 一段下げは**何が重なっても最大 1 段**。
- **D-33**: `HINT_METHOD_DOWNGRADE` に `paper-handwriting: 'kanji-to-kana'` を足し、
  **既存 assert 1 行を書き換える**（D-26 が根拠）。
- **D-34**: 入口の設計係数は **`ENTRY_RULES`（版番号つき）に 1 箇所**。`quick` は 8 問（§6 の表示例が根拠）。
- **D-35**: **`jsdom` に IndexedDB は無い**（実測）。保存層は port 経由。本物への接続は
  `ui/adapters/indexeddb-port.ts` の**1 ファイルだけ**。
- **D-36**: **「回答 → 保存 → 開示」を型で強制する**（`SaveReceipt` を `unique symbol` で封じる）。
- **D-37**: **画像資産を足さない**（公開許可リストに触れる操作である）。文言で伝える。P11 で扱う。

**上のブロックの「作成・変更予定ファイル」からの差分**（意図的なもの）:

- **`HandwritingPad.tsx` を作らない。** 計画本文が「画面手書き欄は**任意**」とし、
  受入条件が「**手書き欄なしでも exam を完了できる**」であるため。**任意のものを先に作らない。**
- **`Guide.tsx` を作らない。** P7 の受入条件のどれにも現れない。
- **`Settings.tsx` を独立画面にしない。** 読み表示・縦横・順序は `RangePicker` と `Session` の中に置く。
  `APP_SPEC` §5 が「**詳細設定を最初から要求せず**、開始後に読み表示と問題数を変更できる」と定めるため。
- **`tests/unit/session-flow.test.ts` の代わりに** `tests/unit/{entry,flow,record,question-schema}.test.ts`
  の 4 本へ分ける（§12.3「1 本に集約しない」）。
- **`data/question-schema.ts` を足した**（上のブロックに無い）。
  出題 JSON の実行時検査が無いままでは画面が生成物を信用することになる。
  **`blankUnit` の 3 値型がコード上どこにも表現されていない**という記録済みの負債もここで解消する。
...
#### P10 の分割（2026-09-01・第14回・裁定 D-38。**後続はこれを前提にすること**）

**P10 を 2 段に割り、P2 だけに依存する部分を先に出した。** 上のブロックは正本のままで、変更していない。

| 段 | 範囲 | 依存 | 状態 |
|---|---|---|---|
| **P10-A（発注031）** | 上の実施内容の **2 のみ**——`review/*.yaml` への追記を行う **CLI**（`tools/review-approve/`） | **P2 だけ** | 発注書作成済み |
| **P10-B（未発注）** | 上の実施内容の **1・3・4・5**——`tools/review-page/` の画面、報告確認、管理者集計、`scan-publish` の検査 | **P9 と裁定 D-02** | P9 完了後 |

**画面より先に書き込み口を固める理由**は、台帳を壊す事故が画面の有無ではなく
**書き込み口の設計で決まる**ためである。P10-A の要点は次のとおり。

- **既定は書き込まない。** `--write` を明示したときだけ書く。
- **一括承認の必須項目を CLI が拒否によって強制する**（`confirmedBy` を `"unknown"` や空文字で埋めない。
  `confirmationMode: batch` なら `batchEvidenceRef` が必須。V-15）。
- **書いた YAML を必ず `tools/build-data/parse-yaml.ts` で読み戻し、深く比較してから書く。**
  同 parser は汎用 YAML ではなく、**対で設計しなければ意味がない。**
- **`seed`（骨組みを敷く）と `set`（承認する）を分ける。**
  **実測（2026-09-01）: `review/blanks.yaml` は `entries: []` である。**
  `reviewCounts` は `missing = expected - entries.length` を pending に足しているため、
  **「行が無い」と「pending と書いてある」が同じ数になる。**
  したがって `seed` は**公開出題を 1 件も増やさず**、生成物も 1 バイトも変えない。
- **候補数を決め打ちしない。** `generateQuestions()` が数えた候補と 1 対 1 に対応させる。

**発注031 は `review/*.yaml` の実物を 1 文字も書き換えない**（試験は一時ディレクトリで行う）。
**承認を実際に押すのは人であり、H-04・H-05 の校正は §5.2 の B のとおり全体完成後である。**
**発注031 は、そのときに人が使う道具を先に作るものであって、校正そのものを始めるものではない。**
...
発注書 §10 の形式に従う。5 件（加えて D-06）。うち **D-01・D-03・D-04・D-05・D-06 は 2026-08-30 に裁定済み**、**残る未決は D-02 のみ**である。
...
| D-04 | 本文用書体（Klee One / 比較候補） | **裁定済み（2026-08-30）**。第一候補（UI = Zen Maru Gothic 500/700、和歌本文 = Klee One 600）で確定。実機比較は P3 の測定へ回し、閾値に触れたら再裁定。下記 D-04 ブロックの追記を参照 |
...
**裁定（2026-08-30・依頼者）**: **第一候補で確定して進める。**
  UI = Zen Maru Gothic 500/700、和歌本文 = Klee One 600（`docs/DESIGN_SYSTEM.md` の指定どおり）。
  依頼者の理由:「目視できるものがありません。ひとまず現在の第一候補で進め」。

  **これは DESIGN_SYSTEM の上書きではない。** 同文書は「self-host・容量・縦書き品質に
  問題がある場合は、源柔ゴシック系と word リポジトリ採用済み書体を比較候補にする」と
  書いており、**第一候補が既定で、比較は問題が出たときの分岐**である。今回はその既定を
  確定させたにすぎない。実質的に下記「代替案 (a)」を採ったことになる。

  **比較を捨てたのではなく、P3 の測定へ移した。** 現時点では比較対象を表示する器
  （P1 の足場）すら存在せず、目視比較は物理的に不可能である。比較のために実装を
  止めるのは順序が逆である。

  再裁定の引き金（**P3 で機械的に測る。目視ではない**）:

  | # | 測るもの | 引き金 |
  |---|---|---|
  | 1 | サブセット後の WOFF2 合計容量（3 ウェイト） | **1 MB を超えたら**再裁定 |
  | 2 | iOS Safari / Android Chrome の縦書き | 列がずれる・行送りが崩れる |
  | 3 | 旧字体・異体字・濁点の字形 | サブセット後に欠落する字がある |

  #1 の閾値の根拠: **本アプリの文字集合は有界である。** 百人一首 100 首の本文・作者・読みと
  UI 文言に限られ、常用漢字全体を積む必要がない。サブセット後は 1 ウェイトあたり
  数百 KB 規模に収まる見込みであり、3 ウェイト合計 1 MB はその見込みに対する上限として
  置いた。**この 1 MB は実測値ではなく見直しの引き金である。** P3 で実測すること。

  #3 は本文が読めなくなる不具合であり、容量とは違って妥協できない。
  対象字は少なくとも 曾禰・躬恒・遍昭 を含む（§2 の実測）。
...
- **裁定が必要な件数**: 8 件（D-01〜D-08）。うち残る未決は **D-02 の 1 件のみ**。加えて人間確認 14 件（H-01〜H-14）。**H-01 は完了済み。H-02 は画像分が解消済み。**

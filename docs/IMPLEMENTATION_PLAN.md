...
| フォント WOFF2 | **`packages/shared/assets/fonts/` を正本**とする | 各公開単位の `prebuild` が `public/fonts/` へコピーする。コピー先は `.gitignore` する |
| feedback 画像 | **リポジトリ直下 `assets/feedback/` のまま動かさない** | 同上のコピー方式。`docs/PUBLISH_MANIFEST.md` §5 がこの位置を許可リストに載せているため移動しない |

理由: WOFF2 の実体を git に二重で持たない。一方 `404.html` は base path 依存で共有できず、複製のほうが安い。
`public/` を Vite の `publicDir` として共有することはできない（`publicDir` は 1 パッケージに 1 つしか指定できない）ため、コピー方式を採る。

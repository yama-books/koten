...
| P8 結果・おすすめ・要確認再試行 | **実施内容 1〜4 は完了・commit 済み。項目 5（History 画面）は発注040 が実装中**（2026-09-03） | 034（P8-A 集計）・036（P8-A2）・037（P8-B 結果画面）・038（P8-C 再開計画）・039（P8-D 再開の配線）を検収。**残件は `History.tsx` と `MasteryMeter.tsx`、および §8.6 の「％・メーター・5 色を同時に」の未達**——`--mastery-*` の 5 トークンが `DESIGN_SYSTEM` にあるのに `tokens.css` へ写されておらず、5 色が実際には出ていない。**いずれも発注040 の範囲**（`docs/HANDOFF.md` §8 の G-1〜G-4） |

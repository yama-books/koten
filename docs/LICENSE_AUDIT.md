...
**ただし、これは 2 回とも機械による確認であり、人間確認 H-02 を代替しない。**
...
|---|---|---|---|
| `correct-maru.png` | 依頼者提供画像を参考に輪郭を整えた透過PNG | CC BY 4.0（列挙されている） | 未確認（人間確認 H-02） |
| `needs-review-check.png` | 同上 | 同上 | 未確認（人間確認 H-02） |
| `perfect-hanamaru.png` | 同上 | 同上 | 未確認（人間確認 H-02） |
...
- フォントについては、本表第2節・第2.3節の一次情報確認により、OFL 1.1の一般条件下でself-host・同梱・サブセット化がいずれも可能であることを示す根拠は揃っている。人間確認で追認できれば、S-2はフォントに関しては解除できる見込み。
- feedback画像3点については、第3節で述べたとおり、CC BY 4.0で提供できる法的根拠が未確立であり、S-2に現に抵触している状態。依頼者への確認（Q1〜Q4）が完了し、Q3で明示的な許可が得られるまでは、この3点を公開物として扱わない。

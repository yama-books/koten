...
## 9. 親検収の記録（**2026-09-04・第33回・親担当。合格**）

**報告の数値は 1 つも転記していない。親担当が全項目を自分で採り直し、M-0〜M-6 を 1 件ずつ当て直した。**
**担当の報告に無かった M-6 も当てた**（報告は M-0〜M-5 までだった）。

### 9.1 §5.1 の A-1〜A-9（**すべて成立**）

| # | 実測 |
|---|---|
| **A-1** | **`M` 4 件 ＋ `?? tools/eol-check/index.ts`**。指定の 5 種類ちょうど（**ただし §9.4 の索引の件を先に処理した**） |
| **A-2** | **13 本を `git add` し直しても `git diff --cached --stat` が空**——**内容を 1 バイトも足していないことの証拠** |
| **A-3** | `i/lf` かつ `w/crlf` が **0 本** |
| **A-4** | manifest の変更行は **`sourceHashes` 4 行と `generatedOn` 1 行だけ**（`2026-09-01` → `2026-09-03`。UTC。裁定 4 のとおり） |
| **A-5** | `git diff --stat -- packages/hyakunin/src/data/generated/` が **1 file changed / 5 insertions / 5 deletions** |
| **A-6** | 新しい 4 値は `git show :<file> \| sha256sum` と **4/4 一致** |
| **A-7** | §1 の 17 件が **17/17 OK・FAILED 0 件** |
| **A-8** | `tools/build-data/`・`.gitattributes`・`tests/` の差分 **0 行** |
| **A-9** | `check:eol` は `package.json` と `ci.yml` に **各 1 回**。**step は `npm ci` の前**に入っている |

### 9.2 §5.3 のゲート（**着手後。親担当が実行**）

| ゲート | 実測 |
|---|---|
| `npm run check:eol` | **走査 1032 件・違反 0 件・終了コード 0**（起票時の見積もり「1031 件前後」＋発注書 051 自身の 1 件） |
| `npm run data:check` | **終了コード 0** |
| `npm run test:node` | **tests 420 / pass 420 / fail 0**（**破壊試験の復元後にもう一度走らせて 420/420**） |
| `npm run test:screen` | **12 files / 88 passed** |
| `npm run scan:publish` | **走査 751 件 / 違反 0 件** |
...
### 9.3 §5.2 の破壊試験（**M-0〜M-7 まで。親担当が 1 件ずつ当てた**）

| # | 実測 | 発注書の期待と一致するか |
|---|---|---|
| **M-0** | **前: 走査 1032・違反 13・exit 1**（13 本を名前で列挙）／**後: 違反 0・exit 0** | **一致。この発注の心臓** |
| **M-1** | `check:eol` が **`百人一首_読み_異同確認.md` の 1 本だけ**を挙げて赤。`data:check` が **`V-14: … (field differs: sourceHashes)`** で赤 | **一致** |
| **M-2** | **fresh clone（`ce31c61`・CRLF 0 本）で補修前 manifest だと `V-14` で exit 1。新 manifest を複写すると exit 0** | **一致。H-19 が閉じたことの本体** |
| **M-3** | 13 本が CRLF の状態で判定条件を `if (false)` にすると **「走査 1032 件、違反 0 件」で exit 0** | **一致。S-6 不発。条件が信号を担っている** |
| **M-4** | 走査対象を `-- review` に狭めると **「検査対象が不足しています（走査 5 件、必要 400 件以上）」で exit 1** | **一致（件数まで同じ）。空走査で緑にならない** |
| **M-5** | `sourceHashes` の 1 文字を書き換えると **`V-14`** で赤 | **一致** |
| **M-6** | 3 番の作者を `柿本人麻呂` → `柿本人麿` にすると **`Error: V-04: fixture mismatch at 3 author`。`V-14` ではない** | **一致。S-7 不発。バイトの経路と中身の経路が別物である証拠** |
| **M-7**（親担当の追加） | **M-6 の状態で `check:eol` は違反 0 件のまま** | **中身の変更は `check:eol` に見えない。**役割の分離が正しい（`check:eol` は改行だけを見張る） |

**復元の確認**——退避した 15 本すべてを `cmp` でバイト比較し、**差異 0 件**。
**復元は `cp` で行った**（記憶 `restore-mutated-file-with-cp`）。復元後に `data:check` 0・`check:eol` 0 件・`test:node` 420/420 を採り直した。

### 9.4 **報告にあった「13 本が `git status` に出る」は欠陥ではなかった（索引の stat キャッシュ）**

担当の報告——「`.git/index.lock` 作成権限がなく、改行のみの 13 ファイルが `git status` 上で変更表示される状態が残っている」。

**親担当が調べた結果、内容は完全に一致していた。** 索引が古い stat を持っていただけである。

| 見たもの | 実測 |
|---|---|
| `git diff --numstat` | **4 ファイルだけ**（13 本は 1 行も出ない） |
| `git diff-files --stat` | **4 files changed** |
| 作業ツリーと索引の blob を `sha256sum` で直接比較（3 本を抽出） | **3/3 完全一致** |
| `git ls-files --debug -- NOTICE` の記録サイズ | **634 バイト**（**CRLF 版**）。**実体は 620 バイト**（LF 版。差 14＝CR 14 個） |

> **`git status` は「サイズが違う」段階で modified と表示する。`git diff` は clean filter を通してから比べるので差分 0 になる。**
> **つまり `git status` の 13 行は、索引を書き直せなかったことの痕跡であって、作業の欠陥ではない。**

**親担当の環境では `.git/index.lock` を作成できたので、13 本を `git add` して索引の stat を更新した。**
**`git diff --cached --stat` が空であることが、この add が内容を 1 バイトも動かしていない証拠である**（A-2）。

**教訓**——**改行の補修では `git status` を信じない。`git diff`（clean filter を通す側）で見る。**
記憶 `worktree-correctness-is-git-diff-not-git-status` に落とした。

### 9.5 残っている限界（**閉じていないもの。正直に書く**）

- **CI 上の `check:eol` はほぼ永久に緑である**（fresh checkout なので）。**H-19 を実際に捕まえるのは開発機での実行だけ**（§0.5）。
- **今後 CRLF で作られるファイルを未然に防ぐものではない。** 持ち込まれた後に、開発機で走らせたときに気づく検査である。
- **`core.autocrlf = true` がこの開発機のローカル設定に入っている。** `.gitattributes` の `eol=lf` が優先されるので
  今日は無害だが、**属性の対象外になったファイルは CRLF で checkout される。** `* text=auto eol=lf` が全域を覆っている限り問題は起きない。

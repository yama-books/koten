...
| `packages/shared/src/styles/tokens.css` | **追記のみ**（`--mastery-*` の 5 行。裁定 3。**既存の行を 1 つも書き換えない**） |
...
### 裁定 3: **`--mastery-*` の 5 行を `tokens.css` へ写す。値は自分で決めない。**

`docs/DESIGN_SYSTEM.md` の色トークン節にある 5 行を、**oklch の値をそのまま**
...
- `packages/hyakunin/src/styles.css` と `packages/shared/src/styles/tokens.css` を読む。
- 前者に現れる `var(--xxx)` の名前をすべて集める。
- **その全部が、2 ファイルのどちらかで `--xxx:` として定義されていること**を assert する。
- **集めた名前が空でないこと**を先に assert する（**空なら検査になっていない**）。

**この試験は着手前に走らせると赤になる**（`--mastery-*` の 5 件が未定義のため）。
...
| **A-25** | **破壊試験 C-0 〜 C-15 を 17 件すべて実施し、各回の出力を報告に貼った** | §5.1。**未実施が 1 件でもあれば不合格である**（発注039 で実際に起きた。§5.1 の前書きを読むこと） |
...
**A-17 の 4 本と A-20 は 1 件以上、A-18 は 5 でなければならない。**
1 つでも外れたら、その出力を完了報告に貼って理由を書くこと。

**この検査は「何を渡しても合格を返す」ものではない。両方向で確かめてある。**

- **A-18 は着手前に `0` を返す**（`--mastery-*` は未定義である。§0.1 の G-3）。
  **着手時に自分で `0` を確かめること。** `5` が返るなら §0.4 の照合が間違っている。
...
> **本発注は C-0 から C-15 まで 17 件ある（C-5 は C-5a と C-5b の 2 件である）。1 件も省かないこと。**
...
| **C-5b** | **着手前**に `tests/unit/css-tokens.test.ts` だけを先に作って走らせる | **赤になる**（`--mastery-*` が未定義のため）。**これが G-3 の実証である。出力を報告に貼ること** |
...
| **C-14** | **走査拡張の自己テスト（陰性）。** C-13 の語を入れたまま、`no-pressure` の走査範囲を `packages/hyakunin/src/ui` だけへ戻す | **緑のままになる。** ＝拡張前は共有層が守られていなかったことの実証。**終わったら両方戻す** |
...
2. **着手前の実測 3 点**——A-18 が `0` を返したこと、A-20 が 0 件だったこと、**C-5b が赤だったこと**。
   **この 3 つは「穴が本当に開いていた」ことの証拠である。出力を貼ること。**
3. **受入条件 A-1 〜 A-25 の判定**（コマンドと出力）。
4. **§5.0 の grep 12 本の出力**（0 件のものも「0 件だった」と書く）。
5. **破壊試験 C-0 〜 C-15 の 17 件すべて**（C-5a・C-5b を別に数える）。

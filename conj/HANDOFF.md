...
## 15. 作業2 進行中チェックポイントA（2026-09-24）

### 状態

作業2「配色の土台（直書き色をCSS変数へ）」へ着手した。**まだ実装前の棚卸し段階**であり、途中着手を未着手扱いにしないためここで記録する。

- 正本: GitHub `yama-books/koten` の `main`
- 作業ブランチ: `codex/conj-work2-color-tokens-20260924`
- 着手時の `conj/index.html` blob: `a487ac57c37b8f61f747e3b44f307fe2c574b7d9`
- `fetch_blob` で現行本文 2,906,916文字を取得できた。
- CSS内の色リテラルを棚卸しし、`:root` の既存基本色を含め82種類の色表現を確認した。
- 既存の `--bg / --card / --ink / --muted / --line / --accent` 等は維持し、巨大なデザイントークン体系には拡張しない。

### 今回、固定色として変更しないもの（ユーザー追加指示）

**ドーナツグラフと記録欄の品詞別カラーは固定し、今回のCSS変数化・配色置換対象から除外する。**

現行値:
- 動詞: `--record-verb:#935568`
- 形容詞: `--record-adj:#b77d55`
- 形容動詞: `--record-adjv:#39756f`
- 助動詞: `--record-aux:#3d566b`

ドーナツグラフはこの4色をJSから読み、`--breakdown-color` と `conic-gradient` に利用している。記録欄の品詞バッジ／フィルタも同じ4色を利用しているため、**色値・対応順・参照ロジックを変更しない。**

### 次の作業

1. 固定色を除外したうえで、残りの直書き色を用途別に分類する。
2. 必要最小限のCSSカスタムプロパティを `:root` に追加する。
3. 既定色を一切変えずに置換する。
4. 作業1 v46ガードを維持するテストと、作業2用の色トークン検査を追加／更新する。
5. 実装後に次のチェックポイントを本HANDOFFへ追記する。

...
## 15. 作業2 進行記録1: 棚卸し開始・固定色境界確定（2026-09-24）

作業2「配色の土台（直書き色をCSS変数へ）」を非ローカルで再開した。

### 再開時に判明したこと

- §14では作業2未着手と記録されていたが、現行mainの `tests/unit/conj-record-screen.test.ts` 末尾に、作業2用と思われるテスト `conj: palette defaults are semantic CSS variables while record POS and donut colors stay fixed` が既に存在する。
- したがって作業2は「完全未着手」ではなく、**テスト側のみ途中着手され、実装が未完の状態**として扱う。
- `conj/index.html` の現行main blob `a487ac57c37b8f61f747e3b44f307fe2c574b7d9` は `fetch_blob` で本文取得に成功した。
- CSS内には82種類の色リテラル（hex / rgba）があり、直書き色の棚卸しを開始した。

### ユーザー追加指示による固定色境界

**ドーナツグラフと記録欄の品詞別カラーは固定色とし、今回変更しない。**
...
- `--record-aux:#3d566b`
- `--record-empty:#e8eef1`
- `.record-donut` の固定ベース色 `#e7f1ee`

これらは後続テーマ切替の対象に含めず、作業2でも値を変えない。変数化の機械的完全化より、この固定色境界を優先する。

### 現時点の停止可能地点

ブランチ `codex/conj-work2-css-vars-20260924` をmainから作成し、本節を最初の進行記録として追加した。以後、色用途の分類・CSS変数化・テスト整合を進める。作業3には進まない。


---

## 15. 作業2 進行中チェックポイント（2026-09-24）

### 着手状態

作業2「配色の土台（直書き色をCSS変数へ）」へ着手した。完全非ローカルで、正本は GitHub `yama-books/koten` の `main`。作業ブランチは `codex/conj-work2-css-vars-20260924`。

### 今回の追加固定条件

依頼者補足により、次の色は **今回変更しない固定色** とする。

- ドーナツグラフの色
- 記録欄の品詞別の色

これらは「直書き色ゼロ」を目的に機械的に変数化しない。作業2では、ページ背景・カード・文字・境界線・アクセント・ボタン・hover/active・正解/不正解/警告・記録画面の一般UI・バッジ等のうち、現行CSSを読んでテーマ切替の土台として意味のある色だけを用途別CSS変数へ整理する。

### 維持条件

- 現行既定配色を変えない。
- 作業1のv46記録画面ガードを維持する。
- 作業3（テーマ切替・コーヒー配色）へ進まない。
- HTML構造・JSロジックは原則変更しない。

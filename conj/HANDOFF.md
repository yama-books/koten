...
## 12. 作業1: 記録画面のPC表示崩れ（2026-09-24）

引継ぎ `conj_handoff_2026-09-24.md` の残作業「作業1」だけを実装した。作業2以降には未着手。

### 作業ブランチ

- `conj-record-layout-fix-20260924`
- 正本: GitHub `yama-books/koten` の `main` から分岐
- ローカルリポジトリは使用していない

### 実測した原因

- v44 の最終上書きで `.record-shell` が 640px まで縮小されていた。
- PCでは `.record-detail-grid` が2列のままなので、「取り組んだ問題の内訳」カード自体が約半幅になる。
- その内部で `.record-overview` がドーナツ＋凡例と統計カードをさらに横並びにしており、凡例側の実幅に対して統計列が重なり得る。
- `.record-stat dt`、`.review-copy small`、`.review-rate span` などに8〜9px指定が残っていた。
- 375〜390pxを含むスマホ幅では、先に定義された1列化を後段の `@media(max-width:700px)` が再び2列へ上書きする競合があった。

### 変更

- `conj/index.html` に v46 の最終オーバーライドを追加。
  - PC: `.record-shell` を最大940pxへ戻し、詳細2列（左430px以上・右440px以上）の最小幅を確保。
  - 内訳: ドーナツ＋凡例と統計カードの列幅を明示し、重なりを防止。
  - 900px以下: 詳細カードを1列化。
  - 460px以下: 要確認カードを確実に1列化。
  - 記録画面の本文系を12px、補足系を10px以上へ引き上げ。
  - 活用種類名は `white-space:nowrap` とし、省略表示にせず1行を維持。
- `tests/unit/conj-record-screen.test.ts` の旧8px前提テストを、v46のレイアウト意図・文字サイズ・スマホ1列化を検査する回帰テストへ更新。

### 現在位置

- コード差分は `conj/index.html` と `tests/unit/conj-record-screen.test.ts` のみ（この記録更新を除く）。
- main へのマージ・push相当の反映はまだ行っていない。
- 次はPRでCIを確認し、作業1の検証結果をここへ追記して停止する。作業2には進まない。

...
## 34. 緊急停止・再開起点（2026-09-19 11:02 JST）

ChatGPT表示の不安定挙動により、ユーザー指示で安全停止。
...
- 公開実例 64 / 120
- 次の未完了: D-19 堤中納言物語5例
- D-19は未確定・未保存。ここから先へ進めない。

再開後に最初に扱う未回答質問:
**「教材化のためには、普段どのように用例を収集するのか？」**

この質問に回答した後、ユーザーが継続を希望した場合にのみD-19へ進む。

判断原則:
- 権利 = 作品×提供元×底本単位
- 語形・引用 = 用例単位
- 典拠不明の異同はCHJ / 中納言優先
- 根拠ある版差はpublicTarget + noteで保持
- 同語形別箇所への差替え禁止
- 推測転記禁止

再開時は ROADMAP → HANDOFF本節 → PROGRESS緊急停止節 → registry → public examples の順で同期する。

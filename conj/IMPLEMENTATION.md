...
## 12. 形容動詞runtime接続（2026-09-19）

`adjv-runtime-adapter.js` をmainの `index.html` へ接続済み。

- 表ドリル用117語幹をJSONから生成
- 既存 `いたづらなり` と重複する1語幹を除き116項目を追加
- 追加項目は `exampleAvailable: false` とし、本文なしでは用例欄を出さない
- runtime読込失敗時は旧埋め込みitemsのみで起動
- 実例索引120件は本文を含まない。画面への実例本文接続は別工程
- CHJ本文・anchorはDriveの公開前監査データに残す

一次資料差分として、助動詞 `たし` の補助活用命令形は空欄に修正済み。補助活用連体形 `たかる` は活用表で維持する。

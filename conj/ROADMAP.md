...
#### D1. 作品単位監査
現在地: **17 / 21作品 監査status確定（publicationReady 12 / partial 4 / blocked 1）、64 / 120例 公開接続済み**
...
3. 平家物語 16
4. 方丈記再訪
5. 十訓抄引用漢詩4例再訪（same-work witness方針）
6. 枕草子保留4例再訪（同一語形を保つ再利用可能版が得られた場合）
7. 更級日記保留1例再訪（同一箇所を含む再利用可能版が得られた場合）
8. 落窪物語再訪（検索可能な公開本文が得られた場合）
...
Phase D以降の本文照合では、中納言表記を校正済み基準形として優先する。

判断順:
1. CHJ sampleId / start / 章題で同一箇所を確認
2. 公開本文との差が表記差のみか判定
3. 文法形が変わる差なら、版・底本・校訂方針の根拠を確認
4. 根拠不明ならCHJを優先し、公開本文側は代替用例にしない
5. 根拠が妥当ならpublicTargetへ異同を保持し、noteに理由を記録

したがって、公開本文探索の目標は「何でも同じ語形を見つける」ことではなく、**CHJで選抜した同一用例を、根拠の明確な公開版で再確認すること**とする。


## 8. PUBLIC / CLASSROOM 二層運用

本アプリは学校の学習目的で使うが、公開範囲によって再利用条件を分ける。
...
原則:
- PD / 明示ライセンス / 許諾済み本文のみ
- article35単独ではPUBLICに昇格しない

### CLASSROOM
当該授業の教員・生徒に限定した非公開利用。

条件を満たす場合、著作権法35条に基づく利用を別途検討できる。
ただし:
- 一般公開しない
- 必要最小限
- 著作権者の利益を不当に害しない
- 公衆送信時は学校設置者側の補償金制度の適用確認
- 提供元規約を別途遵守
- CHJ raw本文は複製しない
